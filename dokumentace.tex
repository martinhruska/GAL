...
Complexity of Algorithm \ref{alg:translate} is $O(3) + O(m)*(O(u)+O(m)*O(v)) = O(m*u + m^2+v)$
where $u,v$ are parameters dependent on specific implementation of tha data structures used in the algorithm.
\end{theorem}
...
\begin{proof}
	...
	Complexity of operation at line $6$ is dependent at representation so we will represented by parameter $u$.
	Cycle at line $7$ is repeated at most $O(m)$ (because there are no more transitions).
	The line $8-12$ are again strongly dependent on representation of automata or implementation so
	we will use parameter $v$ to express complexity of this five lines.
